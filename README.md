...
This repository contains the source code for a Zoho Cliq integration that tracks team tasks. The system consists of **Zoho Cliq Deluge Handler & Schedulers** and **Google Sheets Web App APIs** that log tasks directly inside Google Sheets.
...
graph TD
    ...
    subgraph "Google Sheets (Web Apps)"
        M_Sheet["Morning Spreadsheet & Script"]
        E_Sheet["Evening Spreadsheet & Script"]
    end

    %% Routing updates
    Handler -- "Time 08:00 - 15:00 (GET)" --> M_Sheet
    Handler -- "Time 15:00 - 23:00 (GET)" --> E_Sheet
    ...
    M_Pend -- "GET ?action=getPending" --> M_Sheet
    M_Sheet -- "Returns pending list" --> M_Pend
    M_Pend -- "Tags missing users" --> Channel

    E_Pend -- "GET ?action=getPending" --> E_Sheet
    E_Sheet -- "Returns pending list" --> E_Pend
...
*   **[participation_handler.dg](file:///c:/Users/chand/OneDrive/Desktop/Zoho Bot/participation_handler.dg)**
    ...
    *   **Logic**:
        ...
        4. Validates the current time:
            *   **8:00 AM – 3:00 PM**: Routes the update to the Morning Google Sheet Web App.
            *   **3:00 PM – 11:00 PM**: Routes the update to the Evening Google Sheet Web App.
            *   **Other times**: Responds with an error message restricting updates to working hours.
        5. Calls the corresponding Google Sheet Web App URL and sends a success confirmation back to the Cliq user.
...
*   **[morning_pending_users.dg](file:///c:/Users/chand/OneDrive/Desktop/Zoho Bot/morning_pending_users.dg) & [evening_pending_users.dg](file:///c:/Users/chand/OneDrive/Desktop/Zoho Bot/evening_pending_users.dg)**
    ...
    *   **Logic**:
        1. Calls the corresponding Google Sheet Web App URL with `?action=getPending`.
...
### 2. Google Sheets Web App Scripts
...
*   **[morning_sheet_api.js](file:///c:/Users/chand/OneDrive/Desktop/Zoho Bot/morning_sheet_api.js) & [evening_sheet_api.js](file:///c:/Users/chand/OneDrive/Desktop/Zoho Bot/evening_sheet_api.js)**
    *   **Deployed as**: Web Apps (accessible to "Anyone" with executing authority) linked directly to the Google Spreadsheets.
    ...
    *   **Endpoint Routing (`doGet`)**:
        *   **Standard GET**: Logs a task submission. Reads parameter inputs (`user_id`, `user_name`, `task`) and writes a new row to the spreadsheet.
...
### Step 2: Deploy Google Sheets Web Apps
1. Open your Morning Google Sheet -> Extensions -> **Apps Script**.
2. Paste the code from [morning_sheet_api.js](file:///c:/Users/chand/OneDrive/Desktop/Zoho%20Bot/morning_sheet_api.js) into the editor.
3. Open your Evening Google Sheet -> Extensions -> **Apps Script**.
4. Paste the code from [evening_sheet_api.js](file:///c:/Users/chand/OneDrive/Desktop/Zoho%20Bot/evening_sheet_api.js) into the editor.
5. Customize the variables in both scripts:
...
6. Deploy each script as a Web App:
...
2. Under **Message Handlers** (or your Bot participation settings), implement the logic from [participation_handler.dg](file:///c:/Users/chand/OneDrive/Desktop/Zoho%20Bot/participation_handler.dg).
    *   Paste your morning and evening Google Sheet Web App URLs into `morning_url` and `evening_url`.
